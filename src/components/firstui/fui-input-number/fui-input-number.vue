<template>
    <view class='fui-input__number'>
        <view :class="[disabled || min >= inputValue ? 'fui-number__disabled' : '']" :style='{ minHeight:getMinHeight }'
              class='fui-number__minus' @tap='minus'>
            <view v-if='!custom' :style="{backgroundColor:signColor,width:signWidth+'rpx'}" class='fui-minus__sign'>
            </view>
            <slot></slot>
        </view>
        <input v-model='inputValue' :disabled='disabled' :style="{ color: color, fontSize: size + 'rpx', backgroundColor: backgroundColor, height: height + 'rpx', minHeight: height + 'rpx', width: width + 'rpx',borderRadius:radius+'rpx',marginLeft:margin+'rpx',marginRight:margin+'rpx' }" :type='type' class='fui-number__input'
               @blur='blur' />
        <view :class="[disabled || inputValue >= max ? 'fui-number__disabled' : '']" :style="{minWidth:signWidth+'rpx',minHeight:signWidth+'rpx'}"
              class='fui-number__plus' @tap='plus'>
            <view v-if='!custom'
                  :style="{height:signWidth+'rpx',backgroundColor:signColor,left:isNvue?(signWidth/2+'rpx'):'50%'}"
                  class='fui-plus__sign-col'>
            </view>
            <view v-if='!custom' :style="{width:signWidth+'rpx',backgroundColor:signColor}" class='fui-plus__sign-row'>
            </view>
            <slot name='plus'></slot>
        </view>
    </view>
