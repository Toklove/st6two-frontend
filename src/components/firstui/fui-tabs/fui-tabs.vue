<template>
    <scroll-view :class="{'fui-tabs__fixed':isFixed && !isSticky,'fui-tabs__sticky':isSticky}"
                 :scroll-into-view="scrollInto"
                 :scroll-with-animation="true"
                 :scroll-x="scroll" :show-scrollbar="false" :style="{background:background,zIndex:(isFixed || isSticky)?zIndex:1,top: isFixed || isSticky ? top + 'px' : 'auto'}"
                 class="fui-tabs__scrollbox">
        <view :class="{'fui-tabs__full':!alignLeft}" class="fui-scroll__view">
            <view v-for="(tab, index) in vals" :id="tab.fui_s_id" :key="index"
                  :class="{'fui-tabs__full':!alignLeft}" class="fui-tabs__item" @tap="switchTab(index)">
                <view :class="{'fui-tabs__wrap-disabled':tab.disabled,'fui-tabs__item-column':direction==='column' && tab.icon}"
                      :style="{height:height+'rpx'}"
                      class="fui-tabs__text-wrap">
                    <view v-if="isSlider" :class="{'fui-tabs__line-center':center}"
                          :style="{bottom:bottom +'rpx',left:`-${padding}rpx`,right:`-${padding}rpx`}" class="fui-tabs__line-wrap">
                        <view :class="{'fui-tabs__line-short':short,'fui-tabs__full':!short,'fui-tabs__slider-color':!getSliderBgColor}"
                    ...
                    <image v-if="tab.icon" :class="{'fui-tabs__icon-column':direction==='column'}"
                           :src="tabIndex===index && tab.selectedIcon?tab.selectedIcon:tab.icon" class="fui-tabs__icon">
                    </image>
                    <!-- #ifdef APP-NVUE -->
                    <view class="fui-tabs__text">
                        ...
                        <text :class="{'fui-tabs__selected-color':!getSelectedColor && tabIndex===index,'fui-tabs__text-color':!color && tabIndex!==index}"
                              :style="{fontSize:(tabIndex===index && isNvue? selectedSize:size)+'rpx',color:tabIndex===index?getSelectedColor:color,fontWeight:tabIndex===index?selectedFontWeight:fontWeight,height:height+'rpx',lineHeight:height+'rpx'}"
                        ...
                        <text
                            ...
                            :style="{color:badgeColor,background:getBadgeBgColor,top:getTop+'rpx'}">{{ isDot ? '' : tab.badge }}
                        </text>
                    </view>
                    ...
                    <view :class="{'fui-tabs__selected-color':!getSelectedColor && tabIndex===index,'fui-tabs__text-color':!color && tabIndex!==index}"
                        ...
                        <text
                            v-if="tab.badge"
                            :class="{'fui-tabs__badge-color':!getBadgeBgColor,'fui-tabs__badge-dot':isDot,'fui-tabs__badge':!isDot}"
                            :style="{color:badgeColor,background:getBadgeBgColor}">{{ isDot ? '' : tab.badge }}
                        </text>
                    </view>
                    <!-- #endif -->
                </view>
