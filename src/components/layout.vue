<template>
    <div :class="`${className}`" class="wrap">
        ...
        >
            <FuiIcon
                v-if="layoutData.showTopBarBackBtn" :color="layoutData.backIconColor" name="arrowleft"
                @click="clickBack"
            ></FuiIcon>
        </FuiNavBar>
        ...
        <!-- 空白页面:结束 -->
        <!-- 页面主体:开始 -->
        <div v-else :class="!layoutData.showBodyPadding ? 'py-24px' : ''" class="layout-body flex-auto">
            <slot />
        </div>
        <!-- 页面主体:结束 -->
    </div>
